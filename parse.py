...
def make_rust(instr_dict):
    ...
    for name in instr_dict:
        name2 = name.upper()
        args = [(arg, arg_lut[arg][1]) for arg in instr_dict[name]['variable_fields']]
        mask_match_str += 'const ARGS_{}: &[(&str, usize); {}] = &[{}];\n'.format(
            name2,
            len(args),
            ', '.join(['("{}", {})'.format(arg, pos) for (arg, pos) in args]),
        )
        insts.append('    ("{}", MATCH_{}, ARGS_{}),\n'.format(name.replace('_', '.'), name2, name2))
    mask_match_str += 'pub const INSTRUCTIONS: [(&str, u32, &[(&str, usize)]); {}] = [\n{}];\n'.format(
